...
import { Search, Filter, Download, Calendar, Clock, User, FileText, Printer, Eye, Trash2, RefreshCw, Archive } from 'lucide-react';
...
const EnhancedAdminDashboard = () => {
  ...
  const [orders, setOrders] = useState([
    ...
    },
    {
      ...
      priority: "high",
      notes: "High quality photo paper",
      completedTime: null,
  ...
  const bulkUpdateStatus = (status) => {
    setOrders(orders.map(order => 
      selectedOrders.includes(order.id) 
        ? { ...order, status, completedTime: status === 'completed' ? new Date().toISOString() : null }
        : order
    ));
    setSelectedOrders([]);
  };
  ...
  const Modal = ({ isOpen, onClose, title, size = "max-w-6xl", children }) => {
    ...
    return (
      <div className="fixed inset-0 bg-white bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className={`bg-white rounded-lg shadow-xl ${size} w-full max-h-[90vh] overflow-hidden`}>
  ...
  const stats = {
    ...
    todayOrders: orders.filter(o => new Date(o.uploadTime).toDateString() === new Date().toDateString()).length
  };
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
          <div className="bg-blue-50 p-4 rounded-lg">
            <h3 className="text-blue-800 font-semibold text-sm">Total Orders</h3>
          ...
          <div className="bg-orange-50 p-4 rounded-lg">
            <h3 className="text-orange-800 font-semibold text-sm">Today's Orders</h3>
                  ...
                  <tr key={order.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-4">
                      <div>
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <p className="text-sm text-gray-500 truncate max-w-32" title={order.fileName}>
                    ...
                    <td className="px-4 py-4">
                      <div className="space-y-1">
                        <p className="text-sm">
                          Pages: <span className="font-medium">{order.pages}</span>
                        </p>
                        <div className="flex items-center gap-2">
                          <span className="text-sm">Copies:</span>
                          <button
                            onClick={() => updateOrderCopies(order.id, order.copies - 1)}
                            className="w-6 h-6 bg-gray-200 hover:bg-gray-300 rounded text-sm font-bold text-center "
                          >
                            -
                          </button>
                          <span className="font-medium px-2">{order.copies}</span>
                          <button
                            onClick={() => updateOrderCopies(order.id, order.copies + 1)}
                        ...
                        <div className="text-xs text-gray-500 space-y-1">
                          <p>{order.colorMode} • {order.paperSize}</p>
                      ...
                      <div className="space-y-1">
                        ...
                        {order.completedTime && (
                          <div className="flex items-center gap-1 text-xs text-green-600">
                            ...
                            <span>Done: {new Date(order.completedTime).toLocaleString()}</span>
                          </div>
                        )}
                      </div>
                    </td>
