      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <button
            ...
            disabled={isAddToBasketLoading || isBuyNowLoading}
            className="flex-1 bg-gradient-to-r from-gray-100 to-gray-200 hover:from-gray-200 hover:to-gray-300 text-gray-800 font-semibold py-3 px-6 rounded-xl transition-all duration-200 transform hover:scale-[1.02] border border-gray-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none flex items-center justify-center gap-2"
          >
          ...
          <button
            onClick={onBuyNow}
            disabled={isBuyNowLoading || isAddToBasketLoading}
            className="flex-1 bg-gradient-to-r from-emerald-500 to-green-600 hover:from-emerald-600 hover:to-green-700 text-white font-bold py-3 px-6 rounded-xl transition-all duration-200 transform hover:scale-[1.02] shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none flex items-center justify-center gap-2"
          >
            {isBuyNowLoading ? (
              <>
...
const PrintingServiceHomepage = () => {
  ...
  useEffect(() => {
    const fetchStatus = async () => {
      const status = await fetch(sendUrl + '/api/shop/get');
      const result = await status.json();
      setIsShopOpen(result.status);
  ...
  const [isAddToBasketLoading, setIsAddToBasketLoading] = useState(false);
  ...
  const toggleShopStatus = async () => {
    ...
    const status = await fetch(sendUrl + '/api/shop/update');
    const result = await status.json();
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
            ...
            {[
              ...
              },
              {
                ...
                desc: 'Your files are auto-deleted after printing for your privacy and peace of mind.',
  ...
  const HomePage = () => (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-16">
        ...
        {/* Service Selection - Only show if user is signed in */}
        {user && (
          <div
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 mb-12 sm:mb-16">
          <div className="bg-gradient-to-br from-blue-50 via-indigo-50 to-blue-100 rounded-3xl p-6 sm:p-8 text-center group hover:scale-105 transition-all duration-300 border border-blue-100 shadow-lg hover:shadow-xl">
            ...
            <p className="text-gray-600 text-sm sm:text-base leading-relaxed">
              Your files are auto-deleted after printing for your privacy and
              peace of mind.
            </p>
          </div>
      ...
      {cartItems.length === 0 ? (
        <div className="text-center py-8 sm:py-12">
          ...
          <button
            ...
            className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white font-semibold py-3 px-6 rounded-xl transition-all duration-200 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
          >
            Continue Shopping
          </button>
        </div>
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="border-t pt-4 sm:pt-6 bg-gradient-to-r from-blue-50 to-purple-50 rounded-2xl p-4 sm:p-6 border border-blue-100">
            ...
            <button
              ...
              disabled={isCheckoutLoading}
              className="w-full bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-700 hover:to-green-700 text-white font-bold py-3 sm:py-4 px-6 rounded-xl transition-all duration-200 transform hover:scale-[1.02] shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none flex items-center justify-center gap-2"
            >
              {isCheckoutLoading ? (
                <>
                  <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div>
  ...
  const AdminSignInButton = () => (
    <button
      onClick={() => {
        if (isAdmin) {
          // If already in admin mode, open the admin dashboard
          setAdminDashboardOpen(true);
        } else if (user && user.role == 'admin') {
          // If user is admin but not in admin mode, enable admin mode and open dashboard
          setIsAdmin(true);
          setAdminDashboardOpen(true);
        } else {
          alert('Access Denied');
  ...
  return (
    <div className="min-h-screen">
      ...
      <BillModal
        ...
        onClose={() => {
          if (!isBuyNowLoading && !isAddToBasketLoading) {
            setBillModalOpen(false);
          }
        }}
        ...
        isAddToBasketLoading={isAddToBasketLoading}
      />
      <BasketModal />
      <SignInModal />
      <RegisterModal />
